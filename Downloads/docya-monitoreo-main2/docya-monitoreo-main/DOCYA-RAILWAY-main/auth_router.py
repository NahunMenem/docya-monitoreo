...
def _verify_apple_token(identity_token: str) -> dict:
    """Verifica un identity token de Apple Sign In usando las claves públicas JWKS de Apple."""
    try:
        header = jwt.get_unverified_header(identity_token)
        kid = header.get("kid")
        if not kid:
            raise HTTPException(status_code=401, detail="Token Apple sin key ID en el header")
        ...
        if not matching_key:
            raise HTTPException(status_code=401, detail="Clave pública de Apple no encontrada para este token")
        ...
        for bundle_id in APPLE_BUNDLE_IDS:
            try:
                ...
                print(f"✅ Apple token verificado con audience={bundle_id}")
                return payload
            except Exception as exc:
                last_error = exc

        print(f"⚠️ Apple token inválido. bundles_intentados={APPLE_BUNDLE_IDS} ultimo_error={last_error}")
        raise HTTPException(status_code=401, detail=f"Token Apple inválido: {last_error}")
...
def _ensure_medico_apple_columns(db):
    ...
    cur = db.cursor()
    cur.execute("ALTER TABLE medicos ADD COLUMN IF NOT EXISTS apple_id TEXT")
    cur.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_medicos_apple_id_unique ON medicos (apple_id) WHERE apple_id IS NOT NULL"
    )
    db.commit()
...
class AppleAuthMedicoIn(BaseModel):
    identity_token: str
...
def auth_apple_medico(data: AppleAuthMedicoIn, db=Depends(get_db)):
    """Login/registro con Apple ID para profesionales."""
    _ensure_medico_profile_columns(db)
    _ensure_medico_apple_columns(db)
    try:
        payload = _verify_apple_token(data.identity_token)

        apple_sub = (payload.get("sub") or "").strip()
        # Apple envía el email en el token solo la primera vez; las siguientes veces
        # puede venir vacío o como relay. Usamos el que manda el cliente como fallback.
        email_token = (payload.get("email") or "").lower().strip()
        email = email_token or (data.email or "").lower().strip()
        # El nombre solo llega la primera vez vía el cliente (data.full_name).
        full_name = (data.full_name or "").strip() or "Profesional DocYa"
        apple_password_hash = get_password_hash(
            f"apple-medico::{apple_sub or 'sin-sub'}::{email or 'sin-email'}"
        )
        provisional_matricula = f"APPLE-{(apple_sub or email or 'PRO').upper()[:40]}"

        if not apple_sub:
            raise HTTPException(status_code=400, detail="Apple no devolvió identidad suficiente (sub vacío)")
        ...
        if medico:
            cur.execute(
                ...
                RETURNING id, full_name, email, tipo, validado, matricula_validada,
                          perfil_completo, foto_perfil
                """,
                (apple_sub, email, email, full_name, apple_password_hash, medico["id"]),
            )
            medico = cur.fetchone()
        else:
            cur.execute(
                """
                INSERT INTO medicos (
                    full_name, email, password_hash, apple_id, tipo, matricula,
                    validado, matricula_validada, perfil_completo, acepta_terminos
                )
                VALUES (%s, %s, %s, %s, 'medico', %s, FALSE, FALSE, FALSE, FALSE)
                RETURNING id, full_name, email, tipo, validado, matricula_validada,
                          perfil_completo, foto_perfil
                """,
                (full_name, email, apple_password_hash, apple_sub, provisional_matricula),
            )
            medico = cur.fetchone()
        ...
        raise HTTPException(status_code=500, detail=f"Error en auth Apple profesional: {exc}")
