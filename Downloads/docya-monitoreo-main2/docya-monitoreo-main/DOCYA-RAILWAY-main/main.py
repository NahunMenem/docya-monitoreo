...
CONSULTA_BUSQUEDA_MIN_MINUTES = 5
CONSULTA_BUSQUEDA_MAX_MINUTES = max(
    int(os.getenv("CONSULTA_BUSQUEDA_MAX_MINUTES", str(CONSULTA_BUSQUEDA_MIN_MINUTES))),
    CONSULTA_BUSQUEDA_MIN_MINUTES,
)
_profile_columns_ready = False
_profile_columns_lock = threading.Lock()
_web_intake_columns_ready = False
_web_intake_columns_lock = threading.Lock()
_teleconsulta_columns_ready = False
_teleconsulta_columns_lock = threading.Lock()
...
def _seconds_until_for_log(value):
    if not isinstance(value, datetime):
        return "None"
    try:
        dt = value.replace(tzinfo=ARG_TZ) if value.tzinfo is None else value.astimezone(ARG_TZ)
        return str(int((dt - now_argentina()).total_seconds()))
    except Exception as exc:
        return f"error:{exc}"


def _log_consulta_cancel_trace(origen: str, consulta_id: int, cur=None, extra: str = ""):
    prefix = f"[CANCEL_TRACE:{origen}] consulta={consulta_id}"
    if cur is None:
        print(f"{prefix} {extra}".strip())
        return
    try:
        cur.execute(
            """
            SELECT estado, medico_id, tipo, COALESCE(canal_atencion, 'domicilio'),
                   metodo_pago, mp_status, mp_payment_id, COALESCE(mp_capturado, FALSE),
                   creado_en, asignada_en, expira_en, disponible_notificada_en
            FROM consultas
            WHERE id = %s
            """,
            (consulta_id,),
        ...
        row = cur.fetchone()
        if not row:
            print(f"{prefix} consulta_no_encontrada {extra}".strip())
            return
        (
            estado,
            medico_id,
            ...
            asignada_en,
            expira_en,
            disponible_notificada_en,
        ) = row
        print(
            f"{prefix} estado={estado} medico_id={medico_id} tipo={tipo} canal={canal} "
            f"metodo_pago={metodo_pago} mp_status={mp_status} mp_payment_id={mp_payment_id} "
            f"mp_capturado={mp_capturado} creado_en={_dt_for_log(creado_en)} "
            f"asignada_en={_dt_for_log(asignada_en)} expira_en={_dt_for_log(expira_en)} "
            f"segundos_hasta_expirar={_seconds_until_for_log(expira_en)} "
            f"disponible_notificada_en={_dt_for_log(disponible_notificada_en)} "
            f"ahora_arg={now_argentina().isoformat()} {extra}"
        )
    except Exception as exc:
        print(f"{prefix} error_log={exc} {extra}".strip())
...
def asegurar_estados_consulta_pago():
    estados = ("preparando_pago", "pendiente_de_refund")
    conn = None
    try:
        # Usar una conexion limpia: _connect_with_retry puede ejecutar queries
        # internas y dejar una transaccion abierta, lo que rompe autocommit.
        conn = psycopg2.connect(DATABASE_URL)
        # ALTER TYPE ADD VALUE debe confirmarse antes de usar el valor en otra
        # consulta; autocommit evita que quede dentro de una transaccion larga.
        conn.autocommit = True
        cur = conn.cursor()
        cur.execute("SELECT to_regtype('estado_consulta')")
        if not cur.fetchone()[0]:
            print("estado_consulta no existe; se omite migracion de estados de pago")
            return
        for estado in estados:
            cur.execute(f"ALTER TYPE estado_consulta ADD VALUE IF NOT EXISTS '{estado}'")
        print(f"Estados de pago en estado_consulta verificados: {', '.join(estados)}")
    except Exception as exc:
        print(f"Error verificando estados de pago en estado_consulta: {exc}")
        if conn:
            try:
                conn.rollback()
            except Exception:
...
@app.on_event("startup")
async def migrar_estados_consulta_pago():
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, asegurar_estados_consulta_pago)
...
def asignar_pendiente_a_medico_disponible(cur, medico_id: int):
    ...
    cur.execute("""
        ...
        WHERE c.medico_id IS NULL
          ...
          AND c.tipo = %s
          AND c.disponible_notificada_en IS NULL
    ...
    cur.execute("""
        ...
        SET medico_id = %s,
            asignada_en = NOW(),
            expira_en = NOW() + (%s * INTERVAL '1 minute')
        WHERE id = %s
          AND medico_id IS NULL
          AND estado = 'pendiente'
          AND disponible_notificada_en IS NULL
        RETURNING id
    """, (medico_id, CONSULTA_BUSQUEDA_MAX_MINUTES, consulta_id))
...
def medico_stats(medico_id: int, db=Depends(get_db)):
    ...
    # 3️⃣ Consultas finalizadas esta semana (con precio real)
    cur.execute(f"""
        SELECT id, fin_atencion, metodo_pago, COALESCE(precio_final, 0) AS precio_final
        FROM consultas
        WHERE medico_id = %s
    ...
    cobrado_efectivo_real = 0   # suma de precio_final real de consultas en efectivo

    metodo_contador = {}

    # 4️⃣ Procesar cada consulta con precio real
    for consulta_id, fin_atencion, metodo_pago, precio_final in consultas_finalizadas:
        hora = fin_atencion.time()
        metodo = (metodo_pago or "efectivo").lower().strip()
        es_efectivo = metodo == "efectivo"
        precio = int(precio_final) if precio_final else 0
        ...
        if es_nocturna:
            ...
            ganancias_nocturnas += precio
            if es_efectivo:
                consultas_nocturnas_efectivo += 1
                cobrado_efectivo_real += precio
            else:
                consultas_nocturnas_tarjeta += 1
        else:
            consultas_diurnas += 1
            ganancias_diurnas += precio
            if es_efectivo:
    ...
    cur.execute(f"""
        SELECT
            COALESCE(SUM(CASE
                WHEN LOWER(COALESCE(metodo_pago, 'efectivo')) = 'efectivo'
                THEN monto_total ELSE 0 END), 0) AS efectivo_bruto,
            COALESCE(SUM(CASE
                WHEN LOWER(COALESCE(metodo_pago, 'efectivo')) <> 'efectivo'
                THEN medico_neto ELSE 0 END), 0) AS app_neto,
            COALESCE(SUM(CASE
                WHEN LOWER(COALESCE(metodo_pago, 'efectivo')) = 'efectivo'
                THEN docya_comision ELSE 0 END), 0) AS comision_efectivo
        FROM pagos_consulta
        WHERE medico_id = %s
        AND DATE_TRUNC('week', fecha) = {CURRENT_ARGENTINA_WEEK_SQL}
    """, (medico_id,))
    ...
    pagos_efectivo_bruto = float(pagos_row[0] or 0) if pagos_row else 0
    pagos_app_neto = float(pagos_row[1] or 0) if pagos_row else 0
    pagos_comision_efectivo = float(pagos_row[2] or 0) if pagos_row else 0
    ...
    return {
        ...
        # Montos reales desde pagos_consulta. El efectivo es bruto recibido
        # por el profesional; app/tarjeta es neto antes de compensar saldos.
        "cobrado_efectivo_real": pagos_efectivo_bruto or cobrado_efectivo_real,
        "cobrado_app_neto_real": pagos_app_neto,
        "comision_efectivo_real": pagos_comision_efectivo,
...
def cancelar_busqueda(consulta_id: int, db=Depends(get_db)):
    ...
    cur.execute("""
        SELECT estado, medico_id, mp_payment_id, mp_status, metodo_pago, COALESCE(mp_capturado, FALSE)
        FROM consultas
        WHERE id = %s
    """, (consulta_id,))
    row = cur.fetchone()
    ...
    estado, medico_id, mp_payment_id, mp_status, metodo_pago, mp_capturado = row

    # 2️⃣ Solo se puede cancelar si está pendiente
    if estado != "pendiente":
        _log_consulta_cancel_trace(
            "MANUAL_PACIENTE_IGNORADA_NO_PENDIENTE",
            consulta_id,
            cur,
            f"estado_previo={estado}",
        )
        return {
            "ok": True,
    ...
    _log_consulta_cancel_trace(
        "MANUAL_PACIENTE_PRE_UPDATE",
        consulta_id,
        cur,
        f"estado_previo={estado} medico_previo={medico_id}",
    )
    ...
    cur.execute("""
        ...
        WHERE id = %s
    """, (consulta_id,))
    cancel_update_rowcount = cur.rowcount
    _log_consulta_cancel_trace(
        "MANUAL_PACIENTE_POST_UPDATE",
        consulta_id,
        cur,
        f"rowcount_update_cancelada={cancel_update_rowcount}",
    )

    if mp_payment_id and ((metodo_pago == "saldo_mp") or mp_capturado or mp_status == "approved"):
        try:
            ...
            print(f"💸 Refund por cancelación paciente {consulta_id}: {refund_resp.status_code} {refund_resp.text}")
            if refund_resp.ok:
                cur.execute("""
                    UPDATE consultas
            ...
            print(f"⚠️ Error reembolsando pago por cancelación: {e}")
    elif mp_payment_id and mp_status in ("preautorizado", "authorized", "in_process", "pending"):
        try:
...
def publicar_consulta_domicilio_disponible(db, consulta_id: int, radio_km: float | None = None) -> int:
    ...
    try:
        ...
        cur.execute("""
            UPDATE consultas
            SET expira_en = CASE
                    WHEN expira_en IS NULL
                      OR expira_en < NOW() + (%s * INTERVAL '1 minute')
            ...
            WHERE id = %s
              ...
              AND estado = 'pendiente'
            RETURNING expira_en
        """, (CONSULTA_BUSQUEDA_MAX_MINUTES, CONSULTA_BUSQUEDA_MAX_MINUTES, consulta_id))
        row_expira = cur.fetchone()
        if not row_expira:
            db.rollback()
            return 0
        expira_en = row_expira[0]
        db.commit()  # Confirmar expira_en extendido antes de que timeout_worker lo vea
        ...
        for medico_id, token, distancia in medicos:
            try:
                ...
                enviados += 1
                enviados_debug.append(f"{medico_id}:{float(distancia):.3f}km")
            except Exception as exc:
        ...
        print(f"Debug domicilio disponible {consulta_id}: enviados={enviados} radio={radio}km ids={enviados_debug}")
        return enviados
    except Exception as exc:
        db.rollback()
...
async def procesar_timeouts(db):
    ...
    try:
        ...
        cur.execute("""
            SELECT id, mp_payment_id, mp_status, metodo_pago, COALESCE(mp_capturado, FALSE),
                   creado_en, asignada_en, expira_en, disponible_notificada_en,
                   COALESCE(canal_atencion, 'domicilio')
            FROM consultas
            WHERE estado = 'pendiente'
              AND medico_id IS NULL
              AND expira_en IS NOT NULL
              AND expira_en <= %s
              AND COALESCE(asignada_en, creado_en) <= %s - (%s * INTERVAL '1 minute')
        """, (ahora_arg_naive, ahora_arg_naive, CONSULTA_BUSQUEDA_MAX_MINUTES))
        ...
        for (
            ...
            disponible_notificada_en,
            canal_atencion,
        ) in vencidas_sin_medico:
            _log_consulta_cancel_trace(
                "TIMEOUT_SIN_PROFESIONAL_PRE_UPDATE",
                consulta_id_sin_medico,
                cur,
                (
                    f"ventana_min={CONSULTA_BUSQUEDA_MAX_MINUTES} canal_pre={canal_atencion} "
                    f"creado_pre={_dt_for_log(creado_en)} asignada_pre={_dt_for_log(asignada_en)} "
                    f"expira_pre={_dt_for_log(expira_en)} disponible_pre={_dt_for_log(disponible_notificada_en)}"
                ),
            )
            cur.execute("""
                UPDATE consultas
            ...
            """, (
                ...
                ahora_arg_naive,
                ahora_arg_naive,
                CONSULTA_BUSQUEDA_MAX_MINUTES,
            ))

            timeout_cancel_rowcount = cur.rowcount
            _log_consulta_cancel_trace(
                "TIMEOUT_SIN_PROFESIONAL_POST_UPDATE",
                consulta_id_sin_medico,
                cur,
                f"rowcount_update_cancelada={timeout_cancel_rowcount} ventana_min={CONSULTA_BUSQUEDA_MAX_MINUTES}",
            )

            if timeout_cancel_rowcount == 0:
                print(
                    f"⏳ Timeout ignorado consulta {consulta_id_sin_medico}: "
                    "la búsqueda fue extendida o ya cambió de estado"
                )
                db.rollback()
                continue
            ...
            if mp_payment_id and ((metodo_pago == "saldo_mp") or mp_capturado or mp_status == "approved"):
                try:
                    ...
                    print(f"💸 Refund por timeout sin profesional {consulta_id_sin_medico}: {refund_resp.status_code} {refund_resp.text}")
                    if refund_resp.ok:
                        cur.execute("""
                            UPDATE consultas
                            SET mp_status = 'refunded',
                    ...
                    print(f"⚠️ Error reembolsando pago sin profesional: {exc}")
            elif mp_payment_id and mp_status in ("preautorizado", "authorized", "in_process", "pending"):
                try:
                    cancel_resp = requests.put(
                        f"https://api.mercadopago.com/v1/payments/{mp_payment_id}",
        ...
        for consulta_id, medico_id, expira_en in vencidas:
            ...
            # asignada_en = NOW() (no NULL) para que COALESCE en procesar_timeouts
            # use este timestamp reciente y no caiga en el creado_en viejo,
            # evitando una cancelación prematura en el próximo ciclo.
            cur.execute("""
                UPDATE consultas
                SET medico_id = NULL,
                    asignada_en = NOW(),
                ...
                WHERE id = %s
                  ...
                  AND expira_en IS NOT NULL
                  AND expira_en <= %s
            """, (consulta_id, medico_id, ahora_arg_naive))

            if cur.rowcount == 0:
                print(
                    f"⏳ Timeout ignorado consulta {consulta_id}: "
                    "la asignación fue extendida, aceptada o cambió de estado"
                )
                db.rollback()
                continue
...
def _run_schema_once(flag_name: str, lock: threading.Lock, statements: list[str]):
    """Corre migraciones livianas fuera de la conexion del request."""
        ...
        try:
            ...
            if schema_db is not None:
                try:
                    ...
                    pass
            print(f"⚠️ No se pudo verificar esquema {flag_name}: {exc}")
        finally:
            if schema_db is not None:
                try:
                    schema_db.close()
...
def _ensure_profile_columns():
    _run_schema_once(
        "_profile_columns_ready",
        _profile_columns_lock,
        [
...
def _validar_perfil_paciente_completo(db, paciente_uuid: str):
    ...
    if not paciente_uuid or not paciente_uuid.strip():
        raise HTTPException(status_code=400, detail="paciente_uuid requerido")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="paciente_uuid inválido")
    row = cur.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
...
def _ensure_web_intake_columns(db):
    ...
    _run_schema_once(
        "_web_intake_columns_ready",
        _web_intake_columns_lock,
        [
            "ALTER TABLE users ADD COLUMN IF NOT EXISTS acquisition_channel TEXT",
            "ALTER TABLE consultas ADD COLUMN IF NOT EXISTS canal_origen TEXT",
...
def _ensure_teleconsulta_columns(db):
    _run_schema_once(
        "_teleconsulta_columns_ready",
        _teleconsulta_columns_lock,
        [
            "ALTER TABLE consultas ADD COLUMN IF NOT EXISTS canal_atencion TEXT DEFAULT 'domicilio'",
            "ALTER TABLE consultas ADD COLUMN IF NOT EXISTS video_url TEXT",
            "ALTER TABLE consultas ADD COLUMN IF NOT EXISTS disponible_notificada_en TIMESTAMP",
        ],
    )
...
async def worker_busqueda_consulta_pendiente(
    ...
    print(f"Ventana maxima de busqueda: {CONSULTA_BUSQUEDA_MAX_MINUTES} minutos")
    db_worker = get_db_worker()
    try:
        cur = db_worker.cursor()
        cur.execute("""
            SELECT COALESCE(canal_atencion, 'domicilio'), disponible_notificada_en, estado
            FROM consultas
            WHERE id = %s
        """, (consulta_id,))
        row = cur.fetchone()
        cur.close()
        if row and row[0] == "domicilio" and row[1] is not None and row[2] == "pendiente":
            print(
                f"Consulta {consulta_id} publicada en Consultas; "
                "no se reasigna automaticamente hasta que venza o alguien acepte"
            )
            return

        ciclos = max_ciclos or max(
            1,
            int((CONSULTA_BUSQUEDA_MAX_MINUTES * 60) / max(espera_segundos, 1)),
        )
...
async def solicitar_consulta(
    ...
    if canal_atencion == "teleconsulta":
        ...
        if not row:
            cur.execute("""
                ...
                VALUES (%s,NULL,'pendiente',%s,%s,%s,%s,%s,%s,%s,NOW(), NOW() + (%s * INTERVAL '1 minute'))
        ...
        cur.execute("""
            ...
            VALUES (%s,%s,'pendiente',%s,%s,%s,%s,%s,%s,%s,NOW(), NOW() + (%s * INTERVAL '1 minute'))
            RETURNING id, creado_en
        """, (
            str(data.paciente_uuid),
            profesional_id,
    ...
    if data.metodo_pago in ("tarjeta", "saldo_mp") and consulta_id_previa:
        ...
        # Verificar que la consulta no fue cancelada/reembolsada mientras el paciente pagaba
        cur_check = db.cursor()
        cur_check.execute("SELECT estado, mp_status FROM consultas WHERE id=%s", (consulta_id_previa,))
        previa_row = cur_check.fetchone()
        cur_check.close()
        if previa_row and previa_row[0] in ('cancelada', 'finalizada'):
            raise HTTPException(
                status_code=409,
                detail={
                    "error": "consulta_cancelada",
                    "mensaje": "La consulta fue cancelada. Tu pago será reembolsado automáticamente.",
                    "mp_status": previa_row[1] or "",
                }
            )
        ...
        if not row:
            print(f"⚠️ No hay profesionales inmediatos → mantener búsqueda por {CONSULTA_BUSQUEDA_MAX_MINUTES} minutos")
            ...
            cur.execute("""
                ...
                SET estado='pendiente',
                    ...
                    expira_en = NOW() + (%s * INTERVAL '1 minute')
                WHERE id=%s
            """, (data.metodo_pago, data.tipo, CONSULTA_BUSQUEDA_MAX_MINUTES, consulta_id_previa))
            db.commit()
            ...
            return {
                "consulta_id": consulta_id_previa,
                "estado": "pendiente",
                "mensaje": "Seguimos buscando profesionales disponibles.",
                "profesional": None
            }
        ...
        """, (
            ...
            data.lat, data.lng, data.metodo_pago, data.tipo,
            CONSULTA_BUSQUEDA_MAX_MINUTES,
            consulta_id_previa
        ))
    ...
    if not row:
        cur.execute("""
            ...
            VALUES (%s,NULL,'pendiente',%s,%s,%s,%s,%s,%s,NOW(), NOW() + (%s * INTERVAL '1 minute'))
            RETURNING id, creado_en
        """, (
            str(data.paciente_uuid),
            data.motivo,
...
def consultas_disponibles_cercanas(medico_id: int, db=Depends(get_db)):
    ...
    cur.execute("""
        ...
        WHERE c.estado = 'pendiente'
          ...
          AND c.tipo = %s
          AND COALESCE(c.canal_atencion, 'domicilio') = 'domicilio'
          AND COALESCE(c.disponible_notificada_en, c.asignada_en, c.creado_en)
    ...
    rows = cur.fetchall()
    print(
        f"Consultas disponibles medico={medico_id} tipo={tipo_medico} "
        f"radio={PRO_ASSIGNMENT_RADIUS_KM}km count={len(rows)}"
    )
    return [
        {
            "id": r[0],
...
def aceptar_consulta(
    ...
    cur.execute("""
        SELECT estado, medico_id, mp_payment_id, paciente_uuid, expira_en, metodo_pago, COALESCE(mp_capturado, FALSE)
        FROM consultas
        WHERE id = %s
    """, (consulta_id,))
    row = cur.fetchone()
    ...
    estado_actual, medico_actual, payment_id, paciente_uuid, expira_en, metodo_pago, mp_capturado = row
    ...
    cur.execute("""
        ...
        WHERE id = %s
          ...
          AND (
              expira_en IS NULL
              OR expira_en > NOW()
              OR COALESCE(disponible_notificada_en, asignada_en, creado_en)
                  > NOW() - (%s * INTERVAL '1 minute')
          )
        RETURNING id
    """, (medico_id, consulta_id, medico_id, CONSULTA_BUSQUEDA_MAX_MINUTES))
    ...
    if payment_id and metodo_pago == "saldo_mp":
        cur = db.cursor()
        cur.execute("""
            UPDATE consultas
            SET mp_status = COALESCE(mp_status, 'approved'),
                mp_capturado = TRUE,
                mp_preautorizado = FALSE
            WHERE id = %s
        """, (consulta_id,))
...
@app.post("/consultas/{consulta_id}/modal_abierto")
def consulta_modal_abierto(consulta_id: int, data: dict, db=Depends(get_db)):
    """El médico abrió el modal de la notificación. Extendemos expira_en para darle
    exactamente CONSULTA_BUSQUEDA_MAX_MINUTES desde ahora, sin importar cuándo
    llegó la notificación push."""
    medico_id = int(data.get("medico_id", 0))
    cur = db.cursor()
    cur.execute("""
        UPDATE consultas
        SET expira_en = NOW() + (%s * INTERVAL '1 minute'),
            asignada_en = NOW()
        WHERE id = %s
          AND estado = 'pendiente'
          AND medico_id = %s
    """, (CONSULTA_BUSQUEDA_MAX_MINUTES, consulta_id, medico_id))
    updated = cur.rowcount
    db.commit()
    print(f"📱 Modal abierto por médico {medico_id} consulta {consulta_id} — expira_en extendido {CONSULTA_BUSQUEDA_MAX_MINUTES} min")
    return {"ok": True, "extended": updated > 0}
...
async def rechazar_consulta(
    ...
    try:
        ...
        # asignada_en = NOW() para reiniciar la ventana de 5 min en procesar_timeouts
        liberar_medico_si_presente(cur, medico_id)
        cur.execute("""
            UPDATE consultas
            SET medico_id = NULL,
...
async def timeout_consulta(consulta_id: int, data: dict, db=Depends(get_db)):
    ...
    ahora_arg_naive = now_argentina().replace(tzinfo=None)

    # Ver estado actual de la consulta
    cur.execute("""
        SELECT medico_id, expira_en, (expira_en IS NOT NULL AND expira_en <= %s) AS vencida
        FROM consultas
        WHERE id = %s
    """, (ahora_arg_naive, consulta_id))
    row = cur.fetchone()

    if not row:
        return {"ok": False, "error": "Consulta inexistente"}

    medico_actual, expira_en, vencida = row
    ...
    # Blindaje: el front puede calcular mal la hora local. Solo el reloj de DB
    # decide si vencio; antes de eso no se libera ni se reasigna.
    if not vencida:
        print(
            f"ℹ️ Timeout temprano ignorado consulta {consulta_id} "
            f"médico {medico_id} expira_en={expira_en}"
        )
        return {"ok": True, "reasignado": False, "temprano": True}
...
async def pagos_notificacion(request: Request, db=Depends(get_db)):
    ...
    cur.execute("""
        ...
        RETURNING id, creado_en
    """, (paciente_uuid, medico_id, lat, lng, tipo, CONSULTA_BUSQUEDA_MAX_MINUTES))
...
def obtener_consulta(consulta_id: int, db=Depends(get_db)):
    ...
    cur.execute("""
        SELECT 
            ...
            c.expira_en,
            c.disponible_notificada_en,
            GREATEST(0, CEIL(EXTRACT(EPOCH FROM (c.expira_en - NOW()))))::INT AS segundos_restantes,
    ...
    (
        ...
        expira_en, disponible_notificada_en, segundos_restantes,
    ...
    return {
        ...
        "video_url": video_url,
        "expira_en": str(expira_en) if expira_en else None,
        "expires_at": str(expira_en) if expira_en else None,
        "disponible_notificada_en": str(disponible_notificada_en) if disponible_notificada_en else None,
        "segundos_restantes": int(segundos_restantes or 0),
...
def consulta_activa_paciente(paciente_uuid: str, db=Depends(get_db)):
    ...
    (cid, estado, motivo, direccion, lat, lng, tipo, medico_id,
     ...
     canal_atencion, video_url, tiempo_estimado_raw,
     expira_en, disponible_notificada_en, segundos_restantes) = row
...
@app.get("/medicos/{medico_id}/teleconsulta-stats")
def teleconsulta_stats_medico(medico_id: int, db=Depends(get_db)):
    """Teleconsultas finalizadas esta semana por el médico y estimado de cobro."""
    cur = db.cursor(cursor_factory=RealDictCursor)

    # Teleconsultas finalizadas esta semana
    cur.execute(f"""
        SELECT
            COUNT(*) AS total,
            COUNT(CASE WHEN COALESCE(mp_capturado, FALSE) = TRUE THEN 1 END) AS cobradas_app,
            COUNT(CASE WHEN COALESCE(mp_status, '') = 'refunded' THEN 1 END) AS reembolsadas
        FROM consultas
        WHERE medico_id = %s
          AND tipo = 'teleconsulta'
          AND estado = 'finalizada'
          AND DATE_TRUNC('week', COALESCE(fin_video_at, creado_en)) = {CURRENT_ARGENTINA_WEEK_SQL}
    """, (medico_id,))
    row = cur.fetchone()

    total = int(row["total"] or 0) if row else 0
    cobradas = int(row["cobradas_app"] or 0) if row else 0

    # Tarifa teleconsulta vigente (tomamos la diurna como referencia)
    cur.execute("""
        SELECT COALESCE(monto, 15000) AS monto
        FROM tarifas_consulta
        WHERE tipo = 'teleconsulta_diurna' AND activa = TRUE
        LIMIT 1
    """)
...
def resumen_liquidaciones(db=Depends(get_db)):
    ...
    cur.execute("""
        SELECT
            ...
            COALESCE(m.alias_cbu, '') AS alias_cbu,
            COALESCE(s.saldo, 0) AS saldo,

            -- Domicilio: desde pagos_consulta (consultas presenciales finalizadas)
            COALESCE(dom.cantidad, 0)  AS domicilio_cantidad,
            COALESCE(dom.neto, 0)      AS domicilio_neto,
            COALESCE(dom.comision, 0)  AS domicilio_comision,
            COALESCE(dom.efectivo_cantidad, 0) AS domicilio_efectivo_cantidad,
            COALESCE(dom.efectivo_bruto, 0)    AS domicilio_efectivo_bruto,
            COALESCE(dom.efectivo_comision, 0) AS domicilio_efectivo_comision,
            COALESCE(dom.app_cantidad, 0)      AS domicilio_app_cantidad,
            COALESCE(dom.app_bruto, 0)         AS domicilio_app_bruto,
            COALESCE(dom.app_neto, 0)          AS domicilio_app_neto,
            COALESCE(dom.app_comision, 0)      AS domicilio_app_comision,

            -- Teleconsulta: consultas finalizadas con canal_atencion='teleconsulta'
            COALESCE(tel.cantidad, 0)  AS tele_cantidad,
            COALESCE(tel.neto, 0)      AS tele_neto,
            COALESCE(tel.comision, 0)  AS tele_comision,

            -- Última liquidación
            liq.ultima_liquidacion,
            liq.ultimo_monto
        ...
        LEFT JOIN (
            SELECT
                ...
                SUM(CASE
                    WHEN LOWER(COALESCE(p.metodo_pago, 'efectivo')) = 'efectivo'
                    THEN p.docya_comision ELSE 0 END
            ...
            WHERE COALESCE(c.canal_atencion, 'domicilio') != 'teleconsulta'
            GROUP BY p.medico_id
        ) dom ON dom.medico_id = m.id
        ...
        LEFT JOIN (
            SELECT
                medico_id,
                MAX(fecha)          AS ultima_liquidacion,
                (ARRAY_AGG(monto_pagado ORDER BY fecha DESC))[1] AS ultimo_monto
            FROM liquidaciones_medico
            GROUP BY medico_id
        ) liq ON liq.medico_id = m.id

        WHERE m.validado = TRUE
          AND (
              COALESCE(s.saldo, 0)     != 0
              OR COALESCE(dom.cantidad, 0) > 0
              OR COALESCE(tel.cantidad, 0) > 0
          )
        ORDER BY COALESCE(s.saldo, 0) DESC, m.full_name ASC
    """)
    rows = cur.fetchall()
    cur.close()
...
def historial_liquidaciones(medico_id: int, db=Depends(get_db)):
    """Historial de liquidaciones de un profesional."""
    cur = db.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT id, medico_id, periodo_inicio, periodo_fin,
               monto_pagado, fecha
        FROM liquidaciones_medico
        WHERE medico_id = %s
        ORDER BY fecha DESC
...
def crear_consulta_previa(data: dict, db = Depends(get_db)):
    asegurar_estados_consulta_pago()
    cur = db.cursor()
